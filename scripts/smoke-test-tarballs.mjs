...
const [core, testing, nativePackage, qrcodePackage, reactPackage, solidPackage] = await Promise.all([
...
// The runtime-plugin entrypoint is Bun-only as of upstream 0.4.0; importing it
// from Node must fail with the Bun-only stub error, not a resolution error.
await assert.rejects(import(${JSON.stringify(`${coreName}/runtime-plugin`)}), /Bun-only/)
